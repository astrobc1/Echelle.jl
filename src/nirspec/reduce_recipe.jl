...
function NIRSPECReduceRecipe(;data_input_path, output_path, do_dark=false, do_flat=true, extractor::SpectralExtractor)
    output_path = output_path * split(data_input_path, Base.Filesystem.path_separator)[end-1] * Base.Filesystem.path_separator
    recipe = NIRSPECNIRSPECReduceRecipe(data_input_path, output_path, do_dark, do_flat, extractor)
    return recipe
end

function EchelleReduce.initialize_data(recipe::NIRSPECNIRSPECReduceRecipe)
...
function EchelleReduce.get_master_flat(recipe::NIRSPECNIRSPECReduceRecipe, data, master_flats)
    return _get_master_flat(data, master_flats)
end

function EchelleReduce.get_master_dark(recipe::NIRSPECNIRSPECReduceRecipe, data, master_darks)
...
function EchelleReduce.get_specregion2d(recipe::NIRSPECNIRSPECReduceRecipe, data::SpecData2d{:ishell})
...
EchelleReduce.get_trace_spacing(recipe::NIRSPECNIRSPECReduceRecipe, data::SpecData2d{:ishell}) = 30
EchelleReduce.get_trace_height(recipe::NIRSPECNIRSPECReduceRecipe, data::SpecData2d{:ishell}) = 28
EchelleReduce.get_extract_orders(recipe::NIRSPECNIRSPECReduceRecipe, data::SpecData2d{:ishell}, sregion::SpecRegion2d) = [SpectralRegions.ordermin(sregion):SpectralRegions.ordermax(sregion);]

function EchelleReduce.get_traces(recipe::NIRSPECNIRSPECReduceRecipe, data; xleft=500, xright=2048-510, n_slices=20)
...
function EchelleReduce.create_output_dirs(recipe::NIRSPECNIRSPECReduceRecipe)
    mkpath(recipe.output_path * "trace")
    mkpath(recipe.output_path * "spectra")
    mkpath(recipe.output_path * "calib")
end

function EchelleReduce.reduce(recipe::NIRSPECNIRSPECReduceRecipe)
...
function EchelleReduce.gen_master_calib_images(recipe::NIRSPECNIRSPECReduceRecipe, data)
    if recipe.do_flat
...
function EchelleReduce.extract_image(recipe::NIRSPECNIRSPECReduceRecipe, data::SpecData2d{:ishell}, traces::Vector, sregion::SpecRegion2d, master_flat, master_dark, read_noise=0, extract_orders=nothing)
...
function EchelleReduce.extract(recipe::NIRSPECNIRSPECReduceRecipe, data, traces, sregions)
...
function get_extraction_result(recipe::NIRSPECNIRSPECReduceRecipe, data::SpecData{:ishell}, sregion::SpecRegion2d, reduced_data::Vector)
    extract_orders = get_extract_orders(recipe, data, sregion)
...
function save_reduced_spectrum(recipe::NIRSPECNIRSPECReduceRecipe, data::SpecData2d{:ishell}, sregion::SpecRegion2d, reduced_data::Vector)
...
function group_flats(recipe::NIRSPECNIRSPECReduceRecipe, data)
    flats = data["flats"]
